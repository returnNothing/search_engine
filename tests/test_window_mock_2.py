...
class Test(unittest.TestCase):
    def test_window(self):
        ...
        window_1 = search_engine.Search_Engine('db').position_with_context(2, 'мама', 3, 10)
        ...
        window_2 = search_engine.Search_Engine('db').position_with_context(2, 'поэтому устала', 3, 10)
        self.assertEqual(window_2, {'text.txt': [Window_mock(pos_str=1, left=0, right=17), Window_mock(pos_str=1, left=8, right=24)]})
        del window_2

        window_3 = search_engine.Search_Engine('db').position_with_context(2, 'мама тоже', 3, 10)
        self.assertEqual(window_3, {'text2.txt': [Window_mock(pos_str=0, left=0, right=21), Window_mock(pos_str=0, left=0, right=21)]})
        del window_3

        window_4 = search_engine.Search_Engine('db').position_with_context(2, 'Сегодня вымыла', 3, 10)
        ...
        window_14 = search_engine.Search_Engine('db').emphasize(2, 'мыла было', 3, 10)
        self.assertEqual(window_14, {'text4.txt': ['Мама <b>мыла</b> раму, хотя <b>было</b> уже поздно.']})
        del window_14

        #window_15 = search_engine.Search_Engine('db').emphasize(2, 'Мама мыла раму, хотя было уже поздно')
        #print(window_15)
